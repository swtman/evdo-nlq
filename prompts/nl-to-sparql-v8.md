...
author: manoliss
notes: >
  Forks v7 and changes ONE thing: the "Topic stems" bullet of Rule 16. Stems now
  come from the Snowball Greek stemmer and every hint line carries a ready regex
  with one character class per vowel (`- αλγοριθμ → [αά]λγ[οό]ρ[ιίϊΐ]θμ`), used as
  FILTER(REGEX(LCASE(?var), "pattern")) — one filter for accent-free ALL-CAPS and
  accented mixed-case titles. Replaces v7's `stem | stém` pair, which accented only
  the last vowel. Everything else is v7 unchanged. See "What changed in v8 vs v7"
  in the Notes and ADR-031.
---
...
16. When the user message contains a **"Resolved entities & terms"** block (it follows the question and was derived from it), use the hints it provides:
    ...
    - **Topic stems**: word stems of the question's content words, listed whenever the question has any — including words that also produced a title candidate, so they are available if the question describes a topic. Each line shows a stem, then `→` and the regex pattern to filter with, e.g. `- αλγοριθμ → [αά]λγ[οό]ρ[ιίϊΐ]θμ`. The KG stores some titles accent-free (ALL-CAPS) and some with accents (mixed-case); SPARQL `LCASE()` removes case but not accents, and the pattern has one class per vowel so it matches both. Copy the pattern exactly as given into `FILTER(REGEX(LCASE(?bookTitle), "[αά]λγ[οό]ρ[ιίϊΐ]θμ"))` on `evdx:title` — do not use `CONTAINS` with the bare stem, and do not write your own pattern. Use only stems of words that describe the topic; ignore stems of words that only frame the question (e.g. from "υπάρχει", "ονομάζεται", "διδάσκεται") and stems of words you already bound through a title candidate.
...
Branch `feat/snowball-stemmer` (2026-09-26), ADR-031. Only the "Topic stems" bullet of Rule 16
changed (user decision: one change per prompt version, so an eval difference has one cause). The
worked examples at three levels, COUNT(DISTINCT) and counting universities by name move to v9.

- **Stems from Snowball** instead of the hand-written suffix list: S39 compared 10 stemmers
  (Paice on the UD Greek treebank; CONTAINS reach over the KG titles) — Snowball tied for best,
  CONTAINS recall 0.772 → 0.865 (0.931 with the short-stem fallback, S40c). Words like «αναλυση»
  no longer vanish from the hint (bug F2).
- **One regex per stem** (`→ [αά]λγ[οό]ρ[ιίϊΐ]θμ`) replaces the `αλγορ | αλγόρ` pair, which
  accented only the last vowel: on ex-024 the Snowball stem with that pair found 35 book rows,
  the vowel-class regex 81 (S40a). σ is `[σς]` because GraphDB's `LCASE` writes a word-final ς.
  REGEX took 0.82–1.00× the time of CONTAINS on GraphDB (S40a).
- The few-shot demos that use stems (ex-024, ex-025, ex-026) were rewritten in the same form;
  ex-025's SPARQL now binds ΠΑΝΕΠΙΣΤΗΜΙΟ ΠΕΙΡΑΙΩΣ, the university its question names (it bound
  ΟΙΚΟΝΟΜΙΚΟ ΠΑΝΕΠΙΣΤΗΜΙΟ ΑΘΗΝΩΝ — a gold fix).
